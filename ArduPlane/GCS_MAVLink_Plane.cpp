...
void GCS_MAVLINK_Plane::handle_set_position_target_local_ned(const mavlink_message_t &msg)
    {
        ...
        const bool x_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_X_IGNORE;
        const bool y_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_Y_IGNORE;
        const bool z_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_Z_IGNORE;
        const bool vx_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_VX_IGNORE;
        const bool vy_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_VY_IGNORE;
        const bool vz_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_VZ_IGNORE;
        const bool ax_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_AX_IGNORE;
        const bool ay_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_AY_IGNORE;
        const bool az_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_AZ_IGNORE;
        const bool yaw_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_YAW_IGNORE;
        const bool yaw_rate_ignore = packet.type_mask & POSITION_TARGET_TYPEMASK_YAW_RATE_IGNORE;

#if HAL_QUADPLANE_ENABLED
        const bool velocity_only_command =
            x_ignore && y_ignore && z_ignore &&
            !vx_ignore && !vy_ignore && !vz_ignore &&
            ax_ignore && ay_ignore && az_ignore &&
            yaw_ignore && yaw_rate_ignore;
        const bool supported_velocity_frame =
            packet.coordinate_frame == MAV_FRAME_LOCAL_NED ||
            packet.coordinate_frame == MAV_FRAME_LOCAL_OFFSET_NED;

        if (velocity_only_command && supported_velocity_frame) {
            plane.set_guided_velocity_NED(Vector3f{packet.vx, packet.vy, packet.vz});
            return;
        }
#endif

        // preserve the existing local-offset altitude-only behaviour
